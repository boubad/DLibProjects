...
namespace info {
	...
	public:
		...
		static void convert_color(const InfoColor &c, std::wstring &ss) {
			...
			os << L"rgb(" << (int)c.red << L"," << (int)c.green << L"," << (int)c.blue << L")";
			ss = os.str();
		}
		static void convert_rotate(double angle, double x, double y, std::string &ss) {
			std::stringstream os;
			os << "rotate(" << angle << " " << x << "," << y << ")";
			ss = os.str();
		}
		static void convert_rotate(double angle, double x, double y, std::wstring &ss) {
			std::wstringstream os;
		...
		void draw(const std::wstring &filename)
		{
			if (this->m_pview != nullptr) {
				...
				if ((nRows > 0) && (nCols > 0)) {
					DrawContextParams *pParams = &(this->m_params);
					int k = 8;
					double dx = (double)(pParams->width * k) / (nCols + k * nCols + 3 + 3 *k);
					double dy = (double)(pParams->height * k) / (nRows + k * nRows + 3 + 3 * k);
					pParams->dx = (dist_type)dx;
					pParams->dy = (dist_type)dy;
					pParams->deltax = dx / k;
					pParams->deltay = dy / k:
