...
public:
	EPSMatriceArranger(const STRINGTYPE &sSigle = STRINGTYPE()) :
			m_sigle(sSigle) {
	...
	template<typename T>
	std::future<bool> export_eps(const STRINGTYPE &filename, size_t nRows,
	...
	template<typename T>
	std::future<bool> export_eps(std::ostream &os, size_t nRows, size_t nCols,
