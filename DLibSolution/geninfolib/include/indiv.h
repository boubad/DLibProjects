...
public:
	...
	template<typename XU, typename XSTRING, typename W>
	bool distance(const std::shared_ptr<Indiv<XU, XSTRING> > &oPtr,
			W &res) const {
	...
	bool distance(const Indiv<XU, XSTRING> &other,
			const std::map<U3, double> &weights, W &res) const {
		return (info_global_compute_distance(this->m_center, other.m_center,
				weights, res));
	}	// distance
...
inline std::ostream & operator<<(std::ostream &os,
		const info::Indiv<U, STRINGTYPE> &d) {
