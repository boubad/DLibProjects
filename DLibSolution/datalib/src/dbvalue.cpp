...
{
    DbValueType tt = this->get_dbvalue_type();
    short s = static_cast<short>(tt);
    dlib::serialize(s,os);
    const boost::any &v = this->m_val;
    if (!v.empty())
    {
...
std::istream & DbValue::read_from(std::istream &in)
{
    ...
    DbValueType t = static_cast<DbValueType>(s);
    if (t == DbValueType::boolType)
        {
            bool b;
            dlib::deserialize(b,in);
            this->m_val = b;
        }
        else if (t == DbValueType::charType)
        {
            char x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::unsignedCharType)
        {
            unsigned char x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t == DbValueType::wcharType)
        {
            wchar_t x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t == DbValueType::shortType)
        {
            short x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::unsignedShortType)
        {
            unsigned short x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::intType)
        {
            int x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t == DbValueType::unsignedIntType)
        {
            unsigned int x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::longType)
        {
            long x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::unsignedLongType)
        {
            unsigned long x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::floatType)
        {
            float x;
             dlib::deserialize(x,in);
             this->m_val = x;
        }
        else if (t == DbValueType::doubleType)
        {
            double x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t == DbValueType::stringType)
        {
            std::string x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t ==  DbValueType::wstringType)
        {
            std::wstring x;
            dlib::deserialize(x,in);
            this->m_val = x;
        }
        else if (t == DbValueType::blobType)
        {
            size_t n = 0;
            dlib::deserialize(n,in);
            if (n > 0){
                std::unique_ptr<byte> oMem(new byte[n]);
                byte *pData = oMem.get();
                for (size_t i = 0; i < n; ++i){
                    dlib::deserialize(pData[i], in);
                }// i
                 this->m_val = Blob(pData, n);
            }
        }
